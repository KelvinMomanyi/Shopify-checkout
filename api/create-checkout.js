  ...
  try {
    ...
    for (const item of items) {
      if (!item.variant_id || typeof item.variant_id !== 'number' || item.variant_id <= 0) {
        return res.status(400).json({ error: `Invalid variant_id for item: ${JSON.stringify(item)}` });
      }
      if (!item.custom_price_cents || item.custom_price_cents <= 0) {
        return res.status(400).json({ error: `Invalid custom_price_cents for item: ${JSON.stringify(item)}` });
      }
      if (!item.quantity || item.quantity <= 0) {
        return res.status(400).json({ error: `Invalid quantity for item: ${JSON.stringify(item)}` });
      }
    }

    console.log(`📦 Creating draft order for ${items.length} items...`);

    // Get store currency (defaults to GBP for £ symbol; adjust via env if needed)
    const currencySymbol = process.env.SHOPIFY_CURRENCY_SYMBOL || '£';
    ...
    const lineItems = items.map(item => {
      ...
      if (item.price_breakdown) {
        ...
        // Add price breakdown as line item properties (hidden with _ prefix)
        if (base > 0) {
          properties.push({
            name: '_Base Price',
            value: `${currencySymbol}${(base / 100).toFixed(2)}`
          });
        }
        ...
        if (fabric > 0) {
          properties.push({
            name: '_Fabric Upcharge',
            value: `${currencySymbol}${(fabric / 100).toFixed(2)}`
          });
        }
        ...
        if (additional > 0) {
          properties.push({
            name: '_Additional Options',
            value: `${currencySymbol}${(additional / 100).toFixed(2)}`
          });
        }
      }

      console.log(`  - Item: Variant ${item.variant_id}, Qty: ${item.quantity}, Price: ${currencySymbol}${customPrice}`);
    ...
    const draftOrderData = {
      ...
      use_customer_default_address: true,
      note: 'Custom pricing applied via checkout app',
      // Optional: Apply taxes automatically based on store settings
      tax_exempt: false,
      // Optional: Set currency if multi-currency, but defaults to store currency
      // currency: 'GBP'
    };
    ...
    console.log(`✅ Draft order created: ID ${draftOrder.id}, Name: ${draftOrder.name}, Status: ${draftOrder.status}`);

    // Optional: Calculate taxes if needed (e.g., for preview)
    // const calculatedDraft = await shopify.draftOrder.calculate({ id: draftOrder.id });
    // console.log('💰 Calculated total:', calculatedDraft.total_price_set);

    // Complete the draft order to generate invoice URL (this moves to 'open' status and enables payment)
    const completedDraftOrder = await shopify.draftOrder.complete(draftOrder.id);

    console.log(`✅ Draft order completed. Invoice URL: ${completedDraftOrder.invoice_url}`);
    console.log(`💳 Status: ${completedDraftOrder.status}, Total: ${currencySymbol}${completedDraftOrder.total_price}`);

    // Return the invoice URL (direct link to payment/checkout page)
    res.json({
      success: true,
      checkout_url: completedDraftOrder.invoice_url,
      draft_order_id: draftOrder.id,
      ...
      total_price: completedDraftOrder.total_price,
      total_price_set: completedDraftOrder.total_price_set,  // Includes currency
      line_items_count: lineItems.length,
      note: 'Customer can pay via the invoice URL. Email invoice separately if needed via sendInvoice endpoint.'
    });
